...
def check_1(lst_obj):
    ...
    Сложность: суммируем сложность последовательных действий, получаем O(n).
    """
...
def check_2(lst_obj):
    ...
    Сложность: Итоговая сложность двух вложенных действий равна их произведению,
    следовательно, итоговая сложность будет O(N^2).
    """
    for j in range(len(lst_obj)):          # O(n)
        if lst_obj[j] in lst_obj[j+1:]:    # O(n) условие + in: O(n) + O(n) = O(n) [сложение сложностей]
            return False                   # O(1)
    return True                            # O(1)
